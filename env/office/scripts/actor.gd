...
@onready var mesh : NavigationMesh = get_node("/root/Root/NavigationRegion3D").navigation_mesh
...
func _physics_process( delta: float,  ) -> void:
	...
	if navigator.is_navigation_finished():
		if not end_communication:
			...
			log[ 'data' ] = msg
			print("Motion ", JSON.stringify(log))
...
func walk( target, id ):
	...
	if target == 'random':
		...
		print("Current triangle is " + str(tr) + " and old one is " + str(old_triangle))
		if tr == -1:
			tr = old_triangle
